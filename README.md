...
```
В ней вызывается функция  ```bool isSeparator(char c)```, которая занимается поиском знаков препинания в исходном тексте. На выходе мы получаем новую отформатированную строку.
<hr>
